...
#' ssd_plot_cdf(fits)
ssd_plot_cdf.fitdists <- function(x, average = FALSE, delta = 7, multi_ci = TRUE, multi_est = TRUE, ...) {
  pred <- ssd_hc(x, percent = 1:99, average = average, delta = delta, multi_ci = multi_ci, multi_est = multi_est)
  data <- ssd_data(x)
