...
.ssd_hp_fitdists <- function(x, conc, ci, level, nboot, parallel, ncpus,
  ...
  if (!length(x) || !length(conc)) {
    no <- numeric(0)
    return(as_tibble(data.frame(conc = no, est = no, se = no, 
...
ssd_hp.fitdists <- function(x, conc, ci = FALSE, level = 0.95, nboot = 1000, parallel = NULL, ncpus = 1, average = TRUE, ic = "aicc", ...) {
  chk_string(ic)
...
ssd_hp.fitdistscens <- function(x, conc, ci = FALSE, level = 0.95, nboot = 1000, parallel = NULL, ncpus = 1, average = TRUE, ic = "aic", ...) {
  chk_string(ic)
