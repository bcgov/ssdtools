...
It is intended to be used in conjunction with other R packages such as `readr` to input data, `dplyr` to manipulate data and `ggplot2` to plot data.
The general package design is in keeping with the tidyverse 
...
For cleaner printing of data frames install the `tibble` package
```r
...
boron_pred
```
The data frame of the estimates can then be plotted together with the original data to summarise an analysis.
...
```{r}
ggplot() + 
  geom_hcintersect(xintercept = 1, yintercept = 5/100) 
```    
...
gp <- ggplot(boron_pred, aes_string(x = "est")) + 
  ...
  geom_hcintersect(xintercept = boron_hc5$est, yintercept = 5/100) +
