...
The user can specify a subset of the following distributions
...
The parameter estimates can be extracted using the tidyverse generic `tidy` function.
However, in and off themselves the estimates are not that helpful.
```{r}
tidy(boron_dists)
```
