...
The user can specify a subset of the following `r length(ssd_dists(type = "all"))` distributions
...
```{r}
fits <- ssd_fit_dists(boron_data, dists = c("llogis", "lnorm", "gamma"))
```

## Coefficients

The parameter estimates can be extracted using the tidyverse generic `tidy` function (or the base R generic `coef` function).
```{r}
tidy(fits)
```
...
It is generally more informative to plot the fits using the `autoplot` generic function.
...
Note if `ssd_gof()` is called with `pvalue = TRUE` then the p-values rather than the statistics are returned for the three tests.
...
The `predict` function can be used to generate model-averaged (or if `average = FALSE` individual) estimates.
...
gp <- ggplot(boron_pred, aes_string(x = "est")) +
  ...
  geom_ssdsegment(data = boron_data, aes_string(x = "Conc / 2", xend = "Conc * 2")) +
  geom_ssdpoint(data = boron_data, aes_string(x = "Conc / 2")) +
  geom_ssdpoint(data = boron_data, aes_string(x = "Conc * 2")) +
