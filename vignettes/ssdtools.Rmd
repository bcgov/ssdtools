...
The parameter estimates can be extracted using the `estimates` function.
However, in and off themselves the estimates are not that helpful.
```{r}
estimates(boron_dists)
```

## Plots

It is generally much more informative to plot the fits using the `autoplot` generic function.
As `autoplot` returns a `ggplot` object it can be modified prior to plotting (printing).
```{r, fig.width = 5}
