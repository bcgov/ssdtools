...
The *ssdtools* package produces a plot of the cumulative distributions for several distribution through the use of the *autoplot()* function. 
For example, consider the boron data that ships with the *ssdtools* package.

```{r echo=TRUE, warning=FALSE, message=FALSE}
data(boron_data)
fit <- ssd_fit_dists(boron_data)
fit.plot <- autoplot(fit)
fit.plot
```

This graphic is a *ggplot* object and so can be saved and embellished in the usual way.
For example, suppose we want to superimpose an evironmental concentration cumulative distribution and compute the exposure risk as outlined in Verdonck et al (2003).
...
In particular, suppose that the exposure concentration follows a log-normal distribution with a mean of `r ex.mean.log` and a standard devation of `r ex.sd.log` on the logarithmic scale. 
...
ex.cdf$ex.cdf <- plnorm(ex.cdf$Conc, 
                        meanlog=ex.mean.log, 
                        sdlog=ex.sd.log) # generate the cdf
...
fit.plot + geom_line(data=ex.cdf, aes(x=Conc, y=ex.cdf), color="red", size=2)+
           annotate("text", label=paste("Exposure distribution"),
...
Verdonck, F. A., Aldenberg, T. , Jaworska, J. and Vanrolleghem, P. A. (2003), Limitations of current risk characterization methods in probabilistic environmental risk assessment. Environmental Toxicology and Chemistry, 22: 2209-2213. doi:10.1897/02-435