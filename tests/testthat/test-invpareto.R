...
test_that("invpareto initial shape is MLEs", {
  ...
  expect_equal(
    estimates(fit),
    list(invpareto = list(weight = 1, scale = 1.03299515712949, shape = 4.14668077241))
  )
})
...
test_that("invpareto with extreme data", {
  ...
  expect_equal(
    estimates(fit98),
    list(invpareto = list(weight = 1, scale = 2.61422908501617, shape = 26.0909009531098))
  )

  fit99r <- ssd_fit_dists(data, dists = "invpareto", rescale = TRUE)
  expect_equal(
    estimates(fit99r),
    list(invpareto = list(weight = 1, scale = 1.00038435059807, shape = 26.0278618888664))
  )
})
